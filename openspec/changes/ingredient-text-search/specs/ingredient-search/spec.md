...
When a user searches their recipe collection on web or mobile, the results SHALL include recipes whose title OR ingredients match the search query. No separate UI mode is required — title and ingredient matches appear in a single unified result list.
