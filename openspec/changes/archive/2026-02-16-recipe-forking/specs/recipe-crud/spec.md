...
#### Scenario: Fork action on public recipe
- **WHEN** user A views user B's public recipe
- **THEN** a "Fork" action SHALL be shown alongside "Save"

#### Scenario: Fork attribution on forked recipe
- **WHEN** user A views their own recipe with forked_from_id set
- **THEN** fork attribution SHALL be displayed: "Forked from [title] by [creator]"
...
- **AND** a "Save" action and "Fork" action SHALL be shown
