...
A user SHALL NOT be able to favorite a recipe they have not cooked at least once. The gate SHALL be enforced at the database level via an RLS policy or trigger that checks for at least one cook_log entry for the user+recipe pair.
...
The migration SHALL drop the `is_favorite` column from the `recipes` table. Existing `is_favorite = true` recipes are owned by the user and are implicitly part of their collection — no data migration to `saved_recipes` is needed. Existing `recipe_ratings` rows with `cooked_date IS NOT NULL` SHALL be backfilled into `cook_log`.
