...
- **AND** a "Save" action SHALL be shown
...
#### Scenario: Interaction actions on uncooked recipe
- **WHEN** user views a recipe they have NOT cooked
- **THEN** "Cooked It" action SHALL be available and prominent
- **AND** "Rate" and "Favorite" actions SHALL be disabled or hidden

#### Scenario: Interaction actions on cooked recipe
- **WHEN** user views a recipe they HAVE cooked at least once
- **THEN** "Cooked It" action SHALL be available (to log additional cooks)
- **AND** "Rate" and "Favorite" actions SHALL be enabled

#### Scenario: Cook history section
- **GIVEN** user has cooked recipe X on Jan 15 and Feb 10
- **WHEN** user views recipe X
- **THEN** the detail page SHALL show the cooking history with both dates and any notes

## REMOVED Requirements

### Requirement: Favorites toggle on recipe list
**Reason**: Replaced by the new recipe interaction model. `is_favorite` is replaced by `recipe_favorites` table with cooking gate. The old favorites filter is replaced by "Favorited" and "Want to Cook" filters.
**Migration**: Existing `is_favorite` data is dropped (user confirmed these are "want to cook" status, not "love it"). New `recipe_favorites` table handles the favorited concept with cooking gate.
