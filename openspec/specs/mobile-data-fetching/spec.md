...
- **THEN** it SHALL use `queryKeys.recipes(userId, search)` from `keys.ts`
...
- **WHEN** `fetchRecipes(userId, search)` is called
- **THEN** it SHALL return `Promise<RecipeListItem[]>`

#### Scenario: Recipe detail query function returns typed result
- **WHEN** `fetchRecipeDetail(recipeId, userId)` is called
- **THEN** it SHALL return the full recipe shape with ingredients, tags, ratings, and interaction state
