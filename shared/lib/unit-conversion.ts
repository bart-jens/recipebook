...
const METRIC_TO_IMPERIAL: Record<string, (qty: number) => ConversionResult> = {
  ml: (q) =>
    q >= 236
      ? { quantity: round(q / 236.588), unit: "cups" }
      : q >= 15
        ? { quantity: round(q / 14.787), unit: "tbsp" }
        : { quantity: round(q / 4.929), unit: "tsp" },
  ...
  g: (q) =>
    q >= 453
      ? { quantity: round(q / 453.592), unit: "lb" }
      : { quantity: round(q / 28.35), unit: "oz" },
