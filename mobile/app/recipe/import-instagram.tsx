...
export default function ImportInstagramScreen() {
  ...
  const handleExtract = async () => {
    ...
    try {
      ...
      const response = await fetch(`${API_BASE}/api/extract-instagram`, {
        method: 'POST',
        headers: {
      ...
      if (!response.ok) {
        Alert.alert('Extraction Failed', data.error || 'Could not extract recipe from this post');
        setExtracting(false);
        return;
      }
  ...
  return (
    <>
      <Stack.Screen options={{ headerTitle: 'Import from Instagram' }} />
      <ScrollView
        style={styles.container}
        contentContainerStyle={styles.content}
        keyboardShouldPersistTaps="handled"
      >
        <Text style={styles.heading}>Import from Instagram</Text>
        <Text style={styles.subtitle}>
          Paste a link to an Instagram post with a recipe in the caption. We'll extract the recipe details using AI.
        </Text>
        ...
        <View style={styles.tips}>
          ...
          <Text style={styles.tipText}>The post must be public for extraction to work.</Text>
          <Text style={styles.tipText}>Works best when the recipe is written in the caption (not just in the image).</Text>
